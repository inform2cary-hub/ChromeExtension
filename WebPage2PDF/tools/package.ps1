...
#
# Output:
#   dist\webpage-to-pdf-<version>.zip   (upload this to Chrome Web Store)
#   dist\webpage-to-pdf-<version>.crx   (only with -Crx, for enterprise policy hosting)
#   dist\webpage-to-pdf.pem             (signing key, created once, KEEP IT SECRET)
#
...
# 'store' holds the listing copy and permission justifications for the dashboard,
# which are for the developer and the reviewer, not for the packaged extension.
...
Write-Host "   $copied files staged, all manifest references present"
...
if ($Crx) {
    ...
    $tempProfile = Join-Path $env:TEMP ("ait-pack-" + [Guid]::NewGuid().ToString('N'))
